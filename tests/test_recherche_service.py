...
@pytest.fixture
def competition_pour_recherche(
    competition_ligue1, equipe_psg, equipe_om, france
):
    from src.models import Equipe, Genre
    lyon = Equipe(id=12, nom="Lyon", pays=france, genre=Genre.MASCULIN)

    m1 = Match(
        id=1, date=date(2024, 1, 5), participants=[equipe_psg, equipe_om],
        phase="J1", statut=MatchStatus.TERMINE,
    )
    m2 = Match(
        id=2, date=date(2024, 1, 12), participants=[equipe_psg, lyon],
        phase="J2", statut=MatchStatus.TERMINE,
    )
    m3 = Match(
        id=3, date=date(2024, 2, 1), participants=[equipe_om, lyon],
        phase="J3", statut=MatchStatus.PLANIFIE,
    )
...
class TestRechercheService:
    def test_chercher_participant_par_nom_partiel(
        self, competition_pour_recherche, equipe_psg
    ):
        resultats = RechercheService.chercher_participant_par_nom(
            competition_pour_recherche, "PSG"
        )
        assert equipe_psg in resultats

    def test_chercher_insensible_a_la_casse(
        self, competition_pour_recherche, equipe_psg
    ):
        resultats = RechercheService.chercher_participant_par_nom(
            competition_pour_recherche, "psg"
    ...
    def test_matchs_par_phase(self, competition_pour_recherche):
        matchs = RechercheService.matchs_par_phase(
            competition_pour_recherche, "J1"
        )
        assert len(matchs) == 1
    ...
    def test_confrontation_directe(
        self, competition_pour_recherche, equipe_psg, equipe_om
    ):
    ...
    def test_filtrer_matchs_combine(
        self, competition_pour_recherche, equipe_psg
    ):
