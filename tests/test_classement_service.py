...
class TestClassement310:
    ...
    def test_ignore_matchs_non_termines(self, competition_ligue1, match_psg_om):
        # match_psg_om a statut PLANIFIE par défaut
        competition_ligue1.ajouter_match(match_psg_om)
        classement = ClassementService.classement_par_points_3_1_0(competition_ligue1)
...
class TestClassementVictoires:
    def test_classement_par_victoires(self, competition_avec_matchs, equipe_psg):
        classement = ClassementService.classement_par_victoires(competition_avec_matchs)
        psg_ligne = next(li for li in classement if li["participant"] == equipe_psg)
        assert psg_ligne["victoires"] == 1
